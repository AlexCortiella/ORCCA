...
class Trainer:
    def __init__(
        ...
        self.mu_c_history = np.expand_dims(np.zeros_like(self.model.mu_c.cpu()), axis = 2)
        self.logsigmasq_c_history = np.expand_dims(np.zeros_like(self.model.logsigmasq_c.cpu()), axis = 2)
        ...
        #self.mu_x_history = np.expand_dims(np.zeros((len(self.train_data.dataset), self.model.input_dim)), axis = 2)
        #self.logsigmasq_x_history = np.expand_dims(np.zeros((len(self.train_data.dataset), self.model.input_dim)), axis = 2)
        self.predicted_clusters_history = np.expand_dims(np.zeros(len(self.train_data.dataset)), axis = 1)
    ...
    def _load_variables(self, variables_path):
        ...
        #self.mu_x_history = np.load(os.path.join(variables_path,'mu_x_epoch.npy'))
        #self.logsigmasq_x_history = np.load(os.path.join(variables_path,'logsigmasq_x_epoch.npy'))
    ...
    def _run_epoch(self, epoch, dataset, mode='train'):
        ...
        for step, data in enumerate(dataset):
            ...
            if mode == 'train':
                ...
                self.z_train.append(self.model.params['z'])
                self.predicted_clusters.append(self.model.params['predicted_clusters'])
                
                if epoch == 0:
                    self.train_labels.append(targets.cpu().numpy())
        ...
        if mode == 'train':
            ...
            self.z_train = torch.cat(self.z_train, dim=0)
            self.predicted_clusters = torch.cat(self.predicted_clusters)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs_run, max_epochs):
            ...
            #if self.min_train_loss > train_loss and epoch > 1000:
            ...
            self.pi_history = np.concatenate((self.pi_history,torch.unsqueeze(self.model.params['pi_c'], dim=1).detach().cpu().numpy()), axis = 1)
            self.mu_c_history = np.concatenate((self.mu_c_history,np.expand_dims(self.model.mu_c.detach().cpu(), axis = 2)), axis = 2)
            self.logsigmasq_c_history = np.concatenate((self.logsigmasq_c_history,np.expand_dims(self.model.logsigmasq_c.detach().cpu(), axis = 2)), axis = 2)
            ...
            with torch.no_grad():
                ...
                x_list = [torch.cat([source for source, target in self.train_data], axis = 0).to(self.gpu_id)]
                
                mu, logsigmasq = self.model._encoder_step(x_list, self.model.encoder, self.model.encoder)   
                
                #mu_x, logsigmasq_x = self.model.decoder(self.z_train)   

            #self.mu_history = np.concatenate((self.mu_history,np.expand_dims(mu.cpu(), axis = 2)), axis = 2)
            #self.logsigmasq_history = np.concatenate((self.logsigmasq_history,np.expand_dims(logsigmasq.cpu(), axis = 2)), axis = 2)
            #self.mu_x_history = np.concatenate((self.mu_x_history,np.expand_dims(mu_x.cpu(), axis = 2)), axis = 2)
            #self.logsigmasq_x_history = np.concatenate((self.logsigmasq_x_history,np.expand_dims(logsigmasq_x.cpu(), axis = 2)), axis = 2)
            #self.predicted_clusters_history = np.concatenate((self.predicted_clusters_history,np.expand_dims(self.predicted_clusters.cpu(), axis = 1)), axis = 1)
            ...
            #np.save(self.run_path + '/mu_x_epoch',self.mu_x_history)    
            #np.save(self.run_path + '/logsigmasq_x_epoch',self.logsigmasq_x_history)    
            #np.save(self.run_path + '/predicted_clusters_epoch',self.predicted_clusters_history)             
